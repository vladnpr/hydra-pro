...
class UgvCombatShiftStoreRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // UGV races are managed separately, but we keep the structure if needed for compatibility
            ...
            'new_drones.*.serial_number' => [
                ...
                'string',
                'max:255',
                Rule::unique('ugv_drones', 'serial_number')->whereNull('deleted_at'),
            ...
            'existing_drones.*.id' => 'required_with:existing_drones|exists:ugv_drones,id',
    ...
    {
        return [
            'new_drones.*.serial_number.unique' => 'НРК з серійним номером :input вже існує в базі.',
        ];
    }
}
