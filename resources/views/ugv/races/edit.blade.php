                        ...
                        <div class="form-group">
                                ...
                                @foreach($plans as $plan)
                                    @php
                                        ...
                                        if (is_array(old('ugv_race_plan_ids'))) {
                                            $isChecked = in_array($plan['id'], old('ugv_race_plan_ids'));
                                        } elseif (!empty($race->checkpoints)) {
                                            $isChecked = in_array($plan['id'], array_column($race->checkpoints, 'id'));
                            ...
                            <div id="hidden-plan-inputs">
                                @php
                                    $selectedIds = [];
                                    if (is_array(old('ugv_race_plan_ids'))) {
                                        $selectedIds = old('ugv_race_plan_ids');
                                    } elseif (!empty($race->checkpoints)) {
                                        $selectedIds = array_column($race->checkpoints, 'id');
                                    } elseif ($race->ugv_race_plan_id) {
                                        $selectedIds = [$race->ugv_race_plan_id];
                                    }
                                @endphp
                                @foreach($selectedIds as $id)
                                    <input type="hidden" name="ugv_race_plan_ids[]" value="{{ $id }}">
                                @endforeach
                            </div>
                        </div>
...
@section('js')
    <script>
        ...
        $(document).ready(function() {
            ...
            // Initial call if there are values
