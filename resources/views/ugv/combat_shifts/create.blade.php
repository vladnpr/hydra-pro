...
@section('title', 'Розпочати чергування НРК')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1>Розпочати нове чергування НРК</h1>
        <div>
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('ugv.combat_shifts.store') }}" method="POST" id="create-shift-form">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                                            ...
                                            <div class="crew-member row mb-2">
                                                ...
                                                <div class="col-md-3">
                                                    <select name="crew[{{ $index }}][shift_type]" class="form-control form-control-sm" required>
                                                        <option value="day" {{ ($member['shift_type'] ?? 'both') === 'day' ? 'selected' : '' }}>Денна</option>
                                                        <option value="night" {{ ($member['shift_type'] ?? 'both') === 'night' ? 'selected' : '' }}>Нічна</option>
                                                        <option value="both" {{ ($member['shift_type'] ?? 'both') === 'both' ? 'selected' : '' }}>Обидві</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-1">
                    ...
                    <div class="col-md-6">
                        <div class="card card-info">
                            ...
                            <div class="card-body">
                                ...
                                <h5 class="mt-4">Наявні НРКи на позиції</h5>
                                <div id="existing-drones-container" class="mb-3">
                                    <p class="text-muted small">Оберіть позицію, щоб побачити НРКи</p>
                                </div>

                                <h5 class="mt-4">Додати нові НРКи на позицію</h5>
                                <div id="new-drones-container">
                                    <!-- Нові НРКи будуть додаватися сюди -->
                                </div>
                                <button type="button" class="btn btn-outline-primary btn-sm mt-2" id="add-new-drone">
                                    <i class="fas fa-plus"></i> Додати НРК
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
                ...
                let html = `
                    <div class="new-drone-item border p-2 mb-2">
                        <div class="form-group mb-1">
                            <label class="small">Назва НРК</label>
                            <input type="text" name="new_drones[${droneIndex}][name]" class="form-control form-control-sm" placeholder="Наприклад: UGV 1" required>
                        </div>
                        <div class="form-group mb-1">
            ...
            $('#position_id').change(function() {
                ...
                if (!positionId) {
                    $('#existing-drones-container').html('<p class="text-muted small">Оберіть позицію, щоб побачити НРКи</p>');
                    return;
                }

                $('#existing-drones-container').html('<p class="text-muted small"><i class="fas fa-spinner fa-spin"></i> Завантаження...</p>');

                $.get(`/admin/ugv/drones/by-position/${positionId}`, function(drones) {
                    if (drones.length === 0) {
                        $('#existing-drones-container').html('<p class="text-muted small">На цій позиції немає зареєстрованих НРКів</p>');
                        return;
                    }
                    ...
                    $('#existing-drones-container').html('<p class="text-danger small">Помилка завантаження НРКів</p>');
                });
            });
