...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex">
            @can('manage-ugv')
                @if(!$userActiveShift)
                    <a href="{{ route('ugv.combat_shifts.create') }}" class="btn btn-primary mr-2">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 text-right">
                                @if($userActiveShift->status === 'opened')
                                    <form action="{{ route('ugv.combat_shifts.leave', $userActiveShift->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                ...
                                @endif
                                <form action="{{ route('ugv.combat_shifts.finish', $userActiveShift->id) }}" method="POST" style="display:inline-block;">
                                    @csrf
                        ...
                        <tbody>
                            ...
                            @forelse($shifts as $shift)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('ugv.combat_shifts.show', $shift->id) }}" class="btn btn-primary btn-sm" title="Перегляд">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('ugv.combat_shifts.report', $shift->id) }}" class="btn btn-secondary btn-sm" title="Звіт по залишку">
                                            ...
                                            @can('manage-ugv')
                                                @if($shift->status === 'opened' && !$isUserInShift && !$userActiveShift)
                                                    <form action="{{ route('ugv.combat_shifts.join', $shift->id) }}" method="POST" style="display:inline-block;">
                                                        @csrf
                                                ...
                                                @if($shift->status === 'opened' && $isUserInShift)
                                                    <form action="{{ route('ugv.combat_shifts.leave', $shift->id) }}" method="POST" style="display:inline-block;">
                                                        @csrf
                                                    ...
                                                    <form action="{{ route('ugv.combat_shifts.finish', $shift->id) }}" method="POST" style="display:inline-block;">
                                                        @csrf
                                                ...
                                                <a href="{{ route('ugv.combat_shifts.edit', $shift->id) }}" class="btn btn-info btn-sm" title="Редагувати">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route('ugv.combat_shifts.destroy', $shift->id) }}" method="POST" style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
