...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1>Чергування "{{ $shift->position_name }}"</h1>
        <div>
            <a href="{{ route('air-defence.combat_shifts.index') }}" class="btn btn-default">
                <i class="fas fa-arrow-left"></i> Назад до списку
            </a>
            <a href="{{ route('air-defence.combat_shifts.report', $shift->id) }}" class="btn btn-primary ml-2">
                <i class="fas fa-file-alt"></i> Звіт по залишку
            </a>
            {{-- <a href="{{ route('air-defence.combat_shifts.flights_report', $shift->id) }}" class="btn btn-secondary ml-2">
                <i class="fas fa-paper-plane"></i> Звіт по польотам
            </a> --}}
            @can('manage-air-defence')
                <a href="{{ route('air-defence.combat_shifts.edit', $shift->id) }}" class="btn btn-info ml-2">
                ...
                @if($shift->status === 'opened')
                    ...
                    @if(!$isUserInShift && !$userActiveShift)
                        <form action="{{ route('air-defence.combat_shifts.join', $shift->id) }}" method="POST" style="display:inline-block;" class="ml-2">
                            @csrf
                    ...
                    @if($isUserInShift)
                        <form action="{{ route('air-defence.combat_shifts.leave', $shift->id) }}" method="POST" style="display:inline-block;" class="ml-2">
                            @csrf
                        ...
                        <form action="{{ route('air-defence.combat_shifts.finish', $shift->id) }}" method="POST" style="display:inline-block;" class="ml-2">
                            @csrf
                    ...
                    @endif
                @else
                    <form action="{{ route('air-defence.combat_shifts.reopen', $shift->id) }}" method="POST" style="display:inline-block;" class="ml-2">
                        @csrf
