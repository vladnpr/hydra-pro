    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex">
            @can('manage-air-defence')
                @if(!$userActiveShift)
                    <a href="{{ route('air-defence.combat_shifts.create') }}" class="btn btn-primary mr-2">
                ...
                @endif
            @endcan
            {{-- <a href="{{ route('air-defence.combat_shifts.active_reports') }}" class="btn btn-info">
            ...
            <div class="col-12">
                <div class="card card-outline card-success">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-4">
                                <strong>Позиція:</strong> {{ $userActiveShift->position_name }}
                            </div>
                            <div class="col-md-4">
                                <strong>Початок:</strong> {{ $userActiveShift->started_at }}
                            </div>
                            <div class="col-md-4 text-right">
                                <form action="{{ route('air-defence.combat_shifts.finish', $userActiveShift->id) }}" method="POST" style="display:inline-block;">
                                    @csrf
                        ...
                        <tbody>
                            ...
                            @forelse($shifts as $shift)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('air-defence.combat_shifts.report', $shift->id) }}" class="btn btn-secondary btn-sm" title="Звіт">
                                                <i class="fas fa-file-alt"></i>
                                            </a>
                                            {{-- <a href="{{ route('air-defence.combat_shifts.flights_report', $shift->id) }}" class="btn btn-default btn-sm" title="Звіт по польотам">
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('air-defence.combat_shifts.show', $shift->id) }}" class="btn btn-primary btn-sm" title="Перегляд">
                                                <i class="fas fa-eye"></i>
                                            </a>

                                            @can('manage-air-defence')
                                                @if($shift->status === 'opened' && !$isUserInShift && !$userActiveShift)
                                                    <form action="{{ route('air-defence.combat_shifts.join', $shift->id) }}" method="POST" style="display:inline-block;">
                                                        @csrf
                                                ...
                                                @if($shift->status === 'opened' && $isUserInShift)
                                                    <form action="{{ route('air-defence.combat_shifts.leave', $shift->id) }}" method="POST" style="display:inline-block;">
                                                        @csrf
                                                    ...
                                                    <form action="{{ route('air-defence.combat_shifts.finish', $shift->id) }}" method="POST" style="display:inline-block;">
                                                        @csrf
                                                ...
                                                <a href="{{ route('air-defence.combat_shifts.edit', $shift->id) }}" class="btn btn-info btn-sm" title="Редагувати">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route('air-defence.combat_shifts.destroy', $shift->id) }}" method="POST" style="display:inline-block;">
                                                    @csrf
                                                    @method('DELETE')
