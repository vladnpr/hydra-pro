...
@section('title', 'Розпочати RECON чергування')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1>Розпочати нове RECON чергування</h1>
        <div>
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('recon.combat_shifts.store') }}" method="POST" id="create-shift-form">
                @csrf
                <div class="row">
                    <div class="col-md-6">
                                            ...
                                            <div class="crew-member row mb-2">
                                                ...
                                                <div class="col-md-4">
                                                    <select name="crew[{{ $index }}][shift_type]" class="form-control form-control-sm" required>
                                                        <option value="day" {{ ($member['shift_type'] ?? '') == 'day' ? 'selected' : '' }}>Денна</option>
                                                        <option value="night" {{ ($member['shift_type'] ?? '') == 'night' ? 'selected' : '' }}>Нічна</option>
                                                        <option value="both" {{ ($member['shift_type'] ?? '') == 'both' || !isset($member['shift_type']) ? 'selected' : '' }}>Обидві</option>
                                                    </select>
                                                </div>
                                                <div class="col-md-2">
